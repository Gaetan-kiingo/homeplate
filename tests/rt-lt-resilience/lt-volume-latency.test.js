// tests/rt-lt-resilience/lt-volume-latency.test.js — LT-01 load scenario + LT-02 volume/index
// coverage on the wave-3 read surface (SRS §4.4; NFR-01, NFR-02).
//
// LT-02 asserts three things, cheapest first:
//   • the required index INVENTORY on listings — those index shapes must exist by definition,
//     independently of whichever plan the planner happens to pick for today's dataset, so this
//     check needs no volume data and stays cheap;
//   • the NFR-02 volume dataset gate (>= 10,000 users, >= 1,000 approved listings on ONE
//     America/Los_Angeles day, >= 1,000 bookings) — the same seed's idempotency is asserted by
//     tests/unit/db.test.js against a dedicated volume database;
//   • EXPLAIN ANALYZE at volume of the EXACT production search SQL
//     (src/modules/search/repo.buildSearchQuery / buildCountQuery, exported for precisely this
//     acceptance) across six filter shapes: the planner never sequentially scans listings
//     unjustified. (Historical note: the wave-2 LT lane approximated this with a hand-written
//     predicate, and probed GET /api/users/me while the NFR-01 read paths were unimplemented;
//     both stand-ins were retired when the real measurement below landed.)
//
...
// GATES on is deliberately narrow — see "why the p95 budget is not a suite gate" below. The
// recorded 5-minute measurement run lives in tests/rt-lt-resilience/lt01-run.js; the FR-12
// concurrent-reservation race component of LT-01 lives in lt01-race.test.js.
//
// ---------------------------------------------------------------------------------------------
...
//   2. MEASUREMENT (`LT01_ENFORCE_P95=1 npx jest tests/rt-lt-resilience/lt-volume-latency.test.js`
