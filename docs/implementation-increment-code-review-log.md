...
| **Tests** | Full suite green, deterministic, no `--forceExit` | ✔ Backend **63 suites / 1404 tests** (re-run 2026-09-11 after the NFR-10 guard inversions, the seed change and the price/dashboard endpoint), client **389 tests** (37 files; the host feature added 25, the StrictMode regression 1), both exit 0 under `TEST_STRICT_HANDLES=1` with `maxWorkers: 1`. Coverage (wave-6 lane, full run): **96.05 % lines · 98.87 % functions · 84.65 % branches**, no file below 80 % lines. |
