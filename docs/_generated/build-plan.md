...
### 6.1 Wave 5 — client foundation, planned in detail (coordinator run, 2026-08-26; baseline `0270a01`, clean tree, CI green)

Wave 5 builds the **toolchain and foundation** of the responsive React **web** client (SRS §2.1.2 —
not React Native; SRS wins over SPMP §5.2.1, recorded at WA-9). Feature screens are wave 6; the
NFR-07 audit and the 5-participant study are wave 7. Wave 5 must leave the backend gates exactly as
measured on this tree: **62 suites / 1386 tests exit 0 under `TEST_STRICT_HANDLES=1`**, `npm run
lint` clean, `npm run build` clean, CI green. A wave-5 change that reddens any of those is a failure
of this wave. **Re-baselined at wave-5 verification (2026-08-26):** the verification lanes
legitimately extended the backend suite — client static scans added to
`tests/coverage/coverage-lane.test.js`, plus the new
`tests/adr-conformance/adr-wave5-client-invariants.test.js` — so the canonical gate on the verified
tree is **63 suites / 1397 tests, exit 0 under `TEST_STRICT_HANDLES=1`** (measured 98.03 s). This
is the same growth-by-verification wave 4 recorded (1345 → 1386): "62 / 1386" below is the
`0270a01` baseline figure; zero failures at the canonical count is the gate.

**Run shape: three ordered sub-waves.** The §6 table's three units survive unchanged, but U5-SHELL
executes in two passes over its own files (the house precedent is wave 4A/4B/4C: single-unit waves
where order matters, parallel units only when files and build order are truly independent):

| Sub-wave | Units | Why this order |
|---|---|---|
| **5A** | **U5-SHELL** (alone) | Owns `client/package.json` + root toolchain; nothing client-side can build, lint or test before it lands. |
| **5B** | **U5-API-CLIENT** ∥ **U5-UI-KIT** | Pure additions under disjoint `client/src/` subtrees; both need only 5A's dependency set and vitest config. |
| **5C** | **U5-SHELL-WIRE** (alone; same owner and files as U5-SHELL) | Integrates session state, the UI kit's announcer/tokens into the layout, and lands the a11y harness — edits shell-owned files, so it cannot run beside 5B. |

#### 5A — U5-SHELL (toolchain + scaffold + layout)

Files: `client/{package.json,package-lock.json,vite.config.js,index.html}`,
`client/src/{main,App,routes}.jsx`, `client/src/layout/**` — plus the root toolchain files no other
wave-5 unit may touch: **`.eslintrc.json`**, **root `package.json` + `package-lock.json`**, and
`README.md` (client section only).

1. **Vite + React 18 scaffold**, `client/` as its own npm package (`npm --prefix client ci`). Pinned
   in `client/package.json`: `react` / `react-dom` ^18.3, `react-router-dom` ^6.26; dev:
   `vite` ^5, `@vitejs/plugin-react` ^4, `vitest` ^2, `jsdom`, `@testing-library/react`,
   `@testing-library/user-event`, `@testing-library/jest-dom`. Client tests run as
   `npm --prefix client test` → `vitest run` — **the root `npm test` contract (the backend-only
   Jest suite, `roots: ['<rootDir>/tests']`; 1386 tests at baseline, 1397 after the verification
   lanes' additions) is untouched**; jest.config.js is not edited.
2. **TLS discipline (NFR-03).** The Vite dev server serves HTTPS using
   `certs/dev-{cert,key}.pem` (generated by `scripts/gen-dev-certs.sh`; the script is a
   documented prerequisite). The `/api` proxy targets `https://localhost:3000` and **verifies** the
   self-signed dev certificate by passing `certs/dev-cert.pem` as the CA (a self-signed cert is its
   own CA; SAN covers `localhost`/`127.0.0.1`). `secure: false`, `rejectUnauthorized: false`, and
   `NODE_TLS_REJECT_UNAUTHORIZED` are **banned anywhere in the client toolchain** — if the proxy
   option shape resists CA injection, use an explicit `https.Agent({ ca })`, never verification off.
3. **Routing skeleton that wave 6 extends without editing shared files** (mirror of the backend's
   route-registry convention): `client/src/routes.jsx` discovers
   `client/src/features/*/routes.jsx` via `import.meta.glob` and mounts whatever exists. Wave-6
   units land their own `features/<name>/routes.jsx`; no wave-6 unit edits a wave-5 file. In wave 5
   the discovered set is empty and the shell ships only what is real: a home page (product
   description, session-aware entry links), a 404 route, and a top-level React error boundary.
   No placeholder screens, no dead nav links.
4. **Layout with accessibility built in, not retrofitted** (NFR-07 is audited in wave 7 but built
   now): `<header>` with `<nav aria-label>`, `<main id="main" tabIndex={-1}>`, `<footer>`; a
   skip-to-content link as first focusable element; exactly one `<h1>` per route; `document.title`
   set per route; focus moved to `#main` on route change; viewport meta + responsive layout down to
   320 px with no horizontal scroll. In 5A the layout uses only its own CSS module
   (`client/src/layout/*.module.css`); it adopts the UI-kit tokens in 5C.
5. **The lint gate (hazard 1 — measured, WILL break otherwise).** Root `.eslintrc.json` gains an
   `overrides` entry for `client/**`: `env.browser`, `parserOptions { sourceType: 'module',
   ecmaFeatures: { jsx: true } }`, plugins `react` (+ `settings.react.version: 'detect'`),
   `react-hooks`, `jsx-a11y` with their recommended configs. Client code stays **linted** —
   adding `client/` to `.eslintignore` is forbidden. `.prettierignore` already excludes only
   `client/dist/`; client sources are Prettier-formatted (Prettier 3 parses JSX natively).
6. **One npm install (the MTUT-W3-03 scar).** All root devDependency additions —
   `eslint-plugin-react`, `eslint-plugin-react-hooks`, `eslint-plugin-jsx-a11y`, **and the pinned
   a11y harness `@axe-core/playwright` + `playwright`** (A-NFR07-1, §8.6) — land in ONE
   `npm install` that regenerates `package-lock.json`. Never `npx --yes`. Browser binaries are a
   host step (`npx playwright install chromium`), documented in the README client section.
7. **Build gate:** `scripts/check-build.js` was re-read this run — it walks only `src/` and
   `scripts/` for `.js` files, so client JSX is out of its reach; **no scoping change needed**
   (hazard 4 verified closed). Root `package.json` gains `client:dev` / `client:build` /
   `client:test` convenience scripts; `npm run build` keeps its current meaning.

Acceptance 5A: `npm --prefix client ci && npm --prefix client run build` emits `client/dist/`;
`npm --prefix client test` green; `npm run lint`, `npm run build`, `npm test` (backend, full
suite — 62 / 1386 at baseline, 63 / 1397 re-baselined; see the §6.1 head note) all green on the
same tree; dev server + proxy work against the running API over verified
TLS; grep gates: no `secure:\s*false`, no `rejectUnauthorized`, no `NODE_TLS_REJECT_UNAUTHORIZED`
under `client/` or in root config.

#### 5B — U5-API-CLIENT (`client/src/api/**`, `client/src/session/**`)

The contract layer wave-6 screens consume. Endpoint surface transcribed from
`src/routes/index.js` + the module `routes.js` files on this tree (never invented; a missing
endpoint is a reported deviation, backend files are NOT wave-5 ownership):

- `POST /api/auth/{register,login,logout,resend-verification}`, `GET /api/auth/verify-email`
- `GET|PATCH /api/users/me`, `DELETE /api/users/me`, `POST /api/users/me/export`,
  `GET /api/users/me/export/:id`
- `GET /api/listings/search`, `POST|GET|PATCH /api/listings(/:id)`, listing cancel
- `GET /api/hosts/:id(/reviews)`
- `POST|GET /api/bookings(/:id)`, `POST /api/bookings/:id/{cancel,confirm-completion}`
- `POST|GET /api/bookings/:id/{messages,reviews,safety-alerts}` (safety: alert create/list per
  module routes), `GET /api/moderation/alerts`
- `GET /api/queue`, `POST /api/queue/:id/decision` (moderator), `POST /api/media/uploads`,
  `POST /api/media`, `DELETE /api/media/:id`

Binding behaviours:

1. **Opaque session, zero cookie access.** `fetch` with `credentials: 'include'`; the session
   cookie is HttpOnly and Redis stores only its SHA-256 digest — client code that touches
   `document.cookie` is a defect (grep-gated in tests). Auth state is *inferred*:
   `client/src/session/` exposes `SessionProvider` / `useSession()` →
   `{ user, status: 'unknown'|'authenticated'|'anonymous', login, logout, refresh }`, hydrated
   from `GET /api/users/me` (401 ⇒ anonymous), and every API 401 broadcasts a session-expired
   event that flips the store — never a cookie read.
2. **Typed errors, code first (NFR-07 groundwork).** Every non-2xx surfaces as
   `ApiError { status, code, message, details?, requestId? }` from the API's structured body
   (`NOT_ELIGIBLE` + reason codes, `MEHKO_DAILY_LISTING_LIMIT`, `NO_CAPACITY`,
   `SEARCH_DEGRADED`, `VALIDATION_FAILED`, …) — never a stringified body. Network failure ⇒
   `NETWORK_ERROR`; unparseable body ⇒ `UNEXPECTED_RESPONSE`. Wave-6 screens map codes to
   user-facing messages announced via the UI kit's aria-live region.
3. **Degraded search is a state, not an exception (NFR-09).** `api.search()` returns
   `{ state: 'ok'|'degraded'|'unavailable', listings?, error? }`: 200 ⇒ `ok`; 200 with
   `degraded: true` ⇒ `degraded` (stale-cache adapter answer, per the corrected FR-01 criterion);
   typed 503 `SEARCH_DEGRADED` ⇒ `unavailable` with the user-facing message. Only unexpected
   failures throw.
4. **ADR-010 shape discipline.** Listing/host JSDoc typedefs model the PUBLIC serializer:
   coarsened `{ lat, lng }` + `areaLabel`; `preciseAddress`/`preciseCoordinates` are optional and
   documented as present only for a guest holding a pending/in-progress booking (or a moderator on
   an FR-07 alert). No client code may assume a precise field exists.

Tests (vitest + mocked `fetch`, inside owned dirs): code surfaced from typed bodies; 401 flips the
session store; the three search states; `credentials: 'include'` on every call; the
`document.cookie` grep gate.

#### 5B — U5-UI-KIT (`client/src/ui/**`, `client/src/styles/**`)

Accessible primitives so wave-6 screens inherit WCAG 2.1 AA instead of retrofitting it (the wave-7
audit will audit exactly this kit):

1. `client/src/styles/tokens.css` — CSS custom properties; every text/background pair ships with
   its **measured contrast ratio in a comment, ≥ 4.5:1 for body text, ≥ 3:1 for large text and UI
   boundaries**; visible `:focus-visible` indicator tokens (≥ 3:1 against adjacent colors);
   `prefers-reduced-motion` respected; responsive spacing/type scale; no off-token color literals
   in components (lint/grep-gated).
2. Components (each with vitest + testing-library specs asserting role, accessible name, and
   keyboard behaviour): `Button` (real `<button>`, busy state via `aria-busy` + disabled
   semantics); `TextInput`/`TextArea`/`Select` wrapped by `FormField` — programmatic label
   association, `aria-describedby` for hint + error, `aria-invalid` on error; `ErrorSummary`
   (focusable, links to fields); **`StatusAnnouncer`** — the app-wide `aria-live` region pair
   (`polite` status, `assertive` errors) with a `useAnnounce()` hook, the NFR-07 error-announcement
   channel the API-client error codes feed; `Dialog` (`aria-modal`, focus trap, Escape, focus
   restore); `Spinner` (`role="status"`, text alternative); `Img` — **`alt` required**; decorative
   images only via an explicit `decorative` prop that emits `alt=""`; `Card`, `VisuallyHidden`.
3. Layout-free: the kit imports nothing from `layout/`, `api/`, or `session/` — it is the leaf.

#### 5C — U5-SHELL-WIRE (same files as U5-SHELL, plus `scripts/a11y-audit.js`)

1. Wire the shell: `App.jsx` wraps `SessionProvider`; the layout adopts `styles/tokens.css`, mounts
   `StatusAnnouncer` once, renders session-aware nav (login state from `useSession()`, no auth
   screens yet — those are wave 6); the error boundary announces via the live region.
2. **Replace the fail-on-purpose `test:a11y` with a real harness that still fails honestly.**
   `scripts/a11y-audit.js` (plain Node, NOT under `tests/` — the Jest roots and house test-placement
   rules stay untouched): builds the client, boots `vite preview`, drives it with the **pinned**
   `playwright` + `@axe-core/playwright` at `wcag2a`/`wcag2aa`, audits every route that exists, adds
   the keyboard checks (skip link first, focus-to-main on navigation), prints per-route results —
   then **exits non-zero with a coverage statement: N of the 7 NFR-07 interfaces exist** (0 or 1 in
   wave 5). The gate flips green only when wave 6 lands all seven and wave 7 extends the audit; a
   script that exits 0 without auditing the seven interfaces is the worst outcome (§8.6). If
   chromium cannot be installed on the host, keep the failing explanatory script with an updated
   message and record the deviation — never a green no-op.
3. Final gates on the finished tree, in this order: `npm run lint` → `npm run build` →
   `npm --prefix client run build` → `npm --prefix client test` → full backend
   `TEST_STRICT_HANDLES=1 npm test` (exit 0 with zero failures; 62 / 1386 at the `0270a01`
   baseline, re-baselined to 63 / 1397 by the verification lanes' own additions — §6.1 head note).

#### Wave-5 verification (what the verifier lanes prove; report update follows)

- SRS §2.1.2: a single responsive React **web** app, desktop + mobile viewports (320 px reflow, no
  horizontal scroll), no native app.
- The NFR-07 clauses checkable **without** the wave-7 audit: semantic landmarks, labelled controls,
  visible focus, alt enforcement, aria-live wiring, contrast tokens ≥ 4.5:1, skip link, focus
  management on route change, `document.title` per route.
- Backend gates re-run and green: **63 suites / 1397 tests exit 0 under `TEST_STRICT_HANDLES=1`**
  (98.03 s) — growth over the 62 / 1386 baseline comes only from the verification lanes' own
  additions (coverage-lane client scans; the wave-5 ADR-conformance suite), the same re-baselining
  wave 4 performed at 1345 → 1386; lint; build; the client additions caused zero backend diffs
  (`git status` shows no `src/`, `tests/`, `db/` changes beyond the declared root toolchain files).
- `docs/verification-report.md` updated to cover waves 0–5. **NFR-07 stays `not_implemented`/open**:
  the axe audit of the seven interfaces is wave 7 and the 5-participant study is a human activity —
  wave 5 closes neither, and the report must say so. Accuracy over a clean story.
- House rules unchanged: no `*-reverify`/`*-gaps`/`verify-*` test files; `tests/helpers/outboxScope.js`
  for any outbox drain; every suite closes what it opens in `afterAll` inside a `finally`; no
  `--forceExit`; the globalTeardown 150 ms settle window untouched; migrations append-only (0006 is
  highest); the human team commits.
