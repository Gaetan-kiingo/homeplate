...
`manifest.labelReview.status === 'signed-off'` (Gaetan Rieben, 2026-08-21, v1) — the label gate is
the one NFR-10 gate that has closed. `claimability(…)` with a live model id now returns
`claimable: true`, which means only that the **preconditions** are met: it answers "if a run were
made, could its numbers be claimed?", not "was a run made". No run exists.

**Proposed fix (wave 4).** Build the ADR-002 two-stage pipeline (deterministic pre-filter → Gemini
free tier through the provider-agnostic ADR-007 adapter) and add the `moderation.scan` handler. Then
run **IT-03** against `tests/fixtures/moderation-eval/v1/` with `ALLOW_LIVE_ADAPTERS_IN_TESTS=true`
for that one measurement run, and record the model id **and prompt version** alongside both measured
rates in a `RESULTS.md`. The **ADR-008 human label sign-off** — the gate that used to block this — was
completed on 2026-08-21 (Gaetan Rieben, set version v1), so the remaining conditions are the pipeline,
the live run, the results file, and both rates below 0.05.
...
| Human label sign-off | **`signed-off`** — Gaetan Rieben, 2026-08-21, set version v1 (ADR-008 label gate closed; every other NFR-10 gate is still open) |
